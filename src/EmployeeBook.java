public class EmployeeBook {
    ...
    public  void indexingPay(int indexingPercent) {
    ...
    public  void getDepartMaxMonthPay(int depart) {
        String member = "";
        float maxPay = -1;
        for (int i = 0; i < employees.length; i++) {
    ...
    public  void getDepartMinMonthPay(int depart) {
        String member = "";
        float minPay = Float.MAX_VALUE;
        for (int i = 0; i < employees.length; i++) {
    ...
    public  void getDepartTotalMonthPay(int depart) {
        float totalPay = 0;
        for (int i = 0; i < employees.length; i++) {
    ...
    public  void getDepartMiddleMonthPay(int depart) {
        int count = 0;
        float totalPay = 0;
        for (int i = 0; i < employees.length; i++) {
    ...
    public  void indexingDepartPay(int depart,float indexingPercent) {
    ...
    public  void getDepartInfo(int depart) {
        for (int i = 0; i < employees.length; i++) {
            if (depart == employees[i].getDepartment()) {
    ...
    // //Методы с параметром int Depart
    public  void getMinMonthPay(int payment) {
        String member = "";
        float Pay = 0;
        for (int i = 0; i < employees.length; i++) {
    ...
    public  void getMaxMonthPay(int payment) {
        String member = "";
        float Pay = 0;
        for (int i = 0; i < employees.length; i++) {
    ...
    public  void createEmployee(String fullName,int depart,int pay) {

        for (int i = 0; i < employees.length; i++) {
            if (employees[i]==null){
                employees[i]=new Employee(fullName, depart, pay);
                return;
            }
    ...
    public  void editPayEmployee(String fullName,int pay) {
        ...
        if(!findResult){
            ...
            System.out.println("Зарплата  "+fullName+" изменена");
        }

    }
    //Изменить отдел сотрудника
    public  void editDepartEmployee(String fullName,int depart) {
        boolean findResult=false;
        for (int i = 0; i < employees.length; i++) {
            if (employees[i].getFullName().equals(fullName)){
        ...
        if(!findResult){
            ...
            System.out.println("Отдел  "+fullName+" изменен");
        }

    }
    //Получить Ф. И. О. всех сотрудников по отделам
    public  void getDepartFullName() {
        for (int i = 0; i < employees.length; i++) {
            System.out.println("Отдел :"+employees[i].getDepartment()+","+" Ф.И.О : " + employees[i].getFullName());
        }
